...
var VitalsMenuButton = GObject.registerClass({
    ...
    _initializeMenu() {
        ...
        //Modified
        this._initializeIPAddrMenuItem();
    ...
    // Modified
    _initializeIPAddrMenuItem() {
        // Create the IP Addr menu item
        let ipAddrKey = '_network_ip_addr_';
        let ipAddrItem = new MenuItem.MenuItem(
            Gio.icon_new_for_string(this._sensorIconPath('network', 'icon')),
            ipAddrKey,
            _('IP Addr'),
            _('Checking...'),
            null
        ...
        this._groups['network'].menu.addMenuItem(ipAddrItem);
        this._sensorMenuItems[ipAddrKey] = ipAddrItem;
        
        // Start checking IP address every 5 seconds
        this._ipCheckInterval = GLib.timeout_add_seconds(
    ...
    _checkIPAddress() {
        const interfaces = ['tun0', 'wlan0', 'eth0'];
        ...
        for (let iface of interfaces) {
            try {
                let [success, output] = GLib.spawn_command_line_sync(`ip addr show ${iface}`);
                if (success) {
                    let outputStr = output.toString();
                    // Look for IPv4 address
                    let match = outputStr.match(/inet (\d+\.\d+\.\d+\.\d+)/);
                    if (match && match[1]) {
                        ipAddress = match[1];
                        found = true;
                        break;
                ...
                // Interface might not exist, continue to next one
                continue;
            }
        }
        ...
        let ipAddrKey = '_network_ip_addr_';
        if (this._sensorMenuItems[ipAddrKey]) {
            this._sensorMenuItems[ipAddrKey].value = ipAddress;
            this._sensorMenuItems[ipAddrKey]._valueLabel.text = ipAddress;
        }
        
        // Also update the hot label if it's displayed in the panel
        if (this._hotLabels[ipAddrKey]) {
            this._hotLabels[ipAddrKey].set_text(ipAddress);
        }
    }
    ...
    destroy() {
        ...
        // Modified
        if (this._ipCheckInterval) {
            GLib.Source.remove(this._ipCheckInterval);
        }
